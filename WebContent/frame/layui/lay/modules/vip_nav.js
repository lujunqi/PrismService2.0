            ...
            $.get(addr, data, function (res) {
                ...
                if (res.data) {
                    $(res.data).each(function (k, v) {
                    	...
                    	if (v.subset) {
                    		var a = $('<a href="javascript:;"><i class="layui-icon">' + v.icon + '</i>' + v.text + '<span class="layui-nav-more"></span></a>');
                    		var dl = $('<dl class="layui-nav-child"></dl>');
                    		$(v.subset).each(function (k1, v1) {
                    			var dd = $('<dd><a href="javascript:;">'+v1.text+'</a></dd>');
                    			if(v1.href){
                    				dd = $('<dd><a href="javascript:;" href-url="'+v1.href+'">'+v1.text+'</a></dd>');
                    				dl.append(dd);
                    			}
                    			
                    			if(v1.subset){
                    				var	li2 = $('<dd data-name="home"  class="layui-nav-item"><a href="javascript:" onClick="vShow('+k+','+k1+');">'+v1.text+'<span class="layui-nav-more"></span></a></dd>');
                    				dl.append(li2);
                    		...
                    		var a = $('<a href="javascript:;" href-url="' + v.href + '"><i class="layui-icon">' + v.icon + '</i>' + v.text + '</a>');
                    		if (v.target) {
